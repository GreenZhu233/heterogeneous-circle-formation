    ...
    def run_node_action(context: LaunchContext):
        ...
        for i in range(num):
            ...
            position_control_server = Node(
                package='quadrotor_control', executable='position_control_server',
                output='screen',
                namespace='quad_'+str(i),
                parameters=[{
                    'use_sim_time': True,
                    'self_odom_topic': 'odom',
                    'force_topic': 'gazebo_ros_force',
                    'action_service_name': 'position_control_action'},
                    FindPackageShare('quadrotor_control').find('quadrotor_control') + '/config/pids.yaml'
                ]
            )
            actions.append(position_control_server)
        return actions
